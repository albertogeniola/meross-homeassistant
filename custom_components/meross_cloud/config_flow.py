...
class MerossFlowHandler(config_entries.ConfigFlow, domain=PLATFORM):
    ...
    def _build_prefilled_schema(self,
    ...
    async def async_step_user(self, user_input=None) -> Dict[str, Any]:
        ...
        data_schema = self._build_prefilled_schema(http_endpoint_default=http_api_endpoint,
        ...
        if match is None:
            ...
            return self._show_form(schema=data_schema, errors={"base": "invalid_http_endpoint"})
        ...
        try:
            ...
            return self._show_form(schema=data_schema, errors={"base": "invalid_credentials"})
        except (UnauthorizedException, ConnectTimeout, HTTPError) as ex:
            _LOGGER.error("Unable to connect to Meross HTTP api: %s", str(ex))
            return self._show_form(schema=data_schema, errors={"base": "connection_error"})
        except ClientConnectorSSLError as ex:
            _LOGGER.error("Unable to connect to Meross HTTP api: %s", str(ex))
            return self._show_form(schema=data_schema, errors={"base": "api_invalid_ssl_code"})
